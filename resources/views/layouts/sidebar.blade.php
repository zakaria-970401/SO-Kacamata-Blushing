    ...
    <div class="container-fluid">
        ...
        <ul class="navbar-nav" id="navbar-nav">
                    ...
                    <div class="collapse menu-dropdown" id="sidebarDashboards">
                        <ul class="nav nav-sm flex-column">
                            @if (in_array('master-user', $permission))
                                <li class="nav-item">
                                    <a href="{{ url('master/masterUser') }}" class="nav-link" data-key="t-analytics">
                                        Users
                                    </a>
                                </li>
                            @endif
                            ...
                            @if (in_array('master-stok', $permission))
                                <li class="nav-item">
                                    <a href="{{ url('master/stok') }}" class="nav-link" data-key="t-analytics"> Stok
                                        Item
                            ...
                            @if (in_array('master-menu', $permission))
                                <li class="nav-item">
                                    <a href="{{ url('permission') }}" class="nav-link" data-key="t-analytics"> Manage
                                        Menu
                                    </a>
                                </li>
                            @endif
            ...
            @if (in_array('stop-opname', $permission) and $so->where('status', 1)->count() > 0)
                <li class="nav-item">
                    <a class="nav-link menu-link" href="{{ url('so/stop') }}">
                        <i class="mdi mdi-stop"></i> <span data-key="t-widgets" class="mr-4">Stop Opname</span> <span
                            class="spinner-border flex-shrink-0 ml-4" data-key="t-new"></span>
                    </a>
                </li>
            @endif
            @if (in_array('form-stok-opname', $permission) and $so->where('status', 1)->count() > 0)
                <li class="nav-item">
                    <a class="nav-link menu-link" href="{{ url('so') }}">
                        <i class="mdi mdi-clipboard"></i> <span data-key="t-widgets" class="mr-4">Form Stok
                            Opname</span>
                    </a>
                </li>
            @endif
            @if (in_array('report', $permission))
                ...
                <li class="nav-item">
                    <a class="nav-link menu-link" href="{{ url('report/transaction_report') }}">
                        <i class="mdi mdi-file-sign"></i> <span data-key="t-widgets" class="mr-4">Transaction
                            Report</span>
                    </a>
                </li>
            @endif
